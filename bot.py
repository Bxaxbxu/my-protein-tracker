...
def sgt_now() -> datetime:
    """Current datetime in Singapore time, regardless of server timezone."""
    return datetime.now(SGT)


def sgt_today() -> date:
    """Current date in Singapore time, regardless of server timezone."""
    return sgt_now().date()
...
def add_entry(user_id: int, amount: float, note):
    today = sgt_today()
    now = sgt_now().replace(tzinfo=None)
    conn = get_db()
    conn.run(
